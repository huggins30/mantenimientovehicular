...
-- Registro de gastos por mano de obra / servicios mecánicos
-- ============================================================
CREATE TABLE public.gastos_mano_obra (
  id        BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id   UUID   REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  unidad_id BIGINT REFERENCES public.unidades(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE public.gastos_mano_obra ENABLE ROW LEVEL SECURITY;

-- Políticas: gastos_mano_obra
CREATE POLICY "mano_obra_select" ON public.gastos_mano_obra FOR SELECT USING (auth.uid() = user_id);
