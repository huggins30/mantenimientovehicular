...
// 2. Fraternidad: Sin Movi, sin Colector, Operador 30% y campo Gastos
...
function IncomeFormComun({ unidad }: IncomeFormProps) {
  ...
  if (esSinColector) {
    ...
    colector = 0;
    operador = (total - ahorroUnidad) * 0.30;
...
function IncomeFormFraternidad({ unidad }: IncomeFormProps) {
  ...
  // Lógica Fraternidad: Ahorro de unidad 25%, Operador 30% (sin colector), Gastos se restan al ingreso
  const ahorroUnidad = totalRecaudado * 0.25;
  const operador = (totalRecaudado - ahorroUnidad) * 0.30;
  const gastos = values.gastos || 0;
  const ingresoARegistrar = totalRecaudado - operador - gastos;
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5 col-span-2 pt-1 border-t border-white/5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-rose-400/80">Se resta al ingreso de la unidad</span>
              </div>
              <div className="relative">
